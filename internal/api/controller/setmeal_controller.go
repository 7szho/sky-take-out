...
func (sc *SetMealController) SaveWithDish(ctx *gin.Context) {
	...
	if err != nil {
		global.Log.ErrContext(ctx, "SaveWithDish保存套餐和菜品信息失败! err=%s", err.Error())
		retcode.Fatal(ctx, err, "")
		return
	}
	retcode.OK(ctx, "")
...
func (sc *SetMealController) OnOrClose(ctx *gin.Context) {
	...
	if err != nil {
		global.Log.ErrContext(ctx, "OnOrClose 套餐启用或禁用失败! err=%s", err.Error())
		retcode.Fatal(ctx, err, "")
		return
	}
	retcode.OK(ctx, "")
...
// GetByIdWithDish 根据套餐id获取套餐和内联菜品信息
