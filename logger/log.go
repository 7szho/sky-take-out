...
// Levels 需要监控日志等级, 只有命中列表中的日志等级才会触发 Hook
...
func NewLogger(level string, filePath string) ILog {
	...
	log := &logrus.Logger{
		...
		Level: parseLevel,                           // Debug 日志等级
		Hooks: make(map[logrus.Level][]logrus.Hook), // 初始化 Hook Map, 否则导致 Hook 添加过程中的空指针引用
